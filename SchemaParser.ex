defmodule SchemaParser do
            ...
            _ ->         
                ...
                if openbrackets == closedbrackets do    
                    ...
                    if tables |> length != openbrackets do
                        ...
                        fields = tables
                            |> Enum.map(&Regex.scan(~r{([a-zA-Z][a-zA-Z0-9]+[ \t]*,[ \t]*(([a-zA-Z]+ ?[a-zA-Z]+)|([a-zA-Z]+\([0-9]+\))|([a-zA-Z]+))\s*)}, &1))
                            |> Enum.map(&getFirstNestedElement(&1))
                            |> Enum.map(&trimList(&1))
                            |> Enum.map(&createMapFromStringList(&1))
                    end
                else
                    cond do
    ...
    defp trimList([h | t]) do
        case t do
            [] -> [String.trim(h)]
            _ -> [String.trim(h) | trimList(t)]
        end
    end

    defp createMapFromStringList([h |t]) do
        case t do 
            [] -> Map.new()
            _ -> [k | v] = String.split(h, ~r{[ \t]*,[ \t]*}) 
                Map.put(createMapFromStringList(t), k, v)
        end
    end
end